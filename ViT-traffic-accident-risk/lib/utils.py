...
@torch.no_grad()
def compute_loss_vit(net,dataloader,risk_mask,global_step,device,mode,
                data_type='nyc'):
...
@torch.no_grad()
def predict_and_evaluate_vit(net,dataloader,risk_mask,global_step,scaler,device,mode):
    """predict val/test, return metrics
    ...
    for feature,target_time,graph_feature,label in dataloader:
        ...
        if mode =='vit':
            ...
            prediction_list.append(net(feature).cpu().numpy())
        label_list.append(label.cpu().numpy())
    prediction = np.concatenate(prediction_list, 0)
    label = np.concatenate(label_list, 0)
