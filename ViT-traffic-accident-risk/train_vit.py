    ...
    for epoch in range(1,training_epoch+1):
        ...
        for train_feature,target_time,gragh_feature,train_label in train_loader:
            ...
            if mode == 'vit':
                ...
                l1 = nn.L1Loss()
                l2 = nn.MSELoss()
                loss_l1 = l1(net(train_feature), train_label)
                loss_l2 = l2(net(train_feature), train_label)
                l = loss_l1 + loss_l2
            ...
            #     % (global_step,epoch, batch, training_loss/len, time() - start_time),flush=True)
            
            batch+=1
            global_step+=1

        print('epoch: %s, training loss: %.6f, time: %.2fs'
                % (epoch, training_loss/len(train_loader), time() - start_time),flush=True)
        #compute va/test loss
        val_loss = compute_loss_vit(net,val_loader,risk_mask,global_step-1,device,mode,data_type)
        # print('global step: %s, epoch: %s, val loss：%.6f' %(global_step-1,epoch,val_loss),flush=True)
        print('epoch: %s, val loss：%.6f' %(epoch,val_loss),flush=True)
        ...
        if epoch == 1 or val_loss < early_stop.best_score:
            test_rmse,test_recall,test_map,test_inverse_trans_pre,test_inverse_trans_label = \
                        predict_and_evaluate_vit(net, test_loader, risk_mask, global_step-1, scaler, device,mode)
           
            high_test_rmse,high_test_recall,high_test_map,_,_ = \
                        predict_and_evaluate_vit(net, high_test_loader, risk_mask, global_step-1, scaler, device,mode)
            ...
            print('epoch: %s, test RMSE: %.4f, test Recall: %.2f%%, test MAP: %.4f\nhigh test RMSE: %.4f, high test Recall: %.2f%%, high test MAP: %.4f'
                % (epoch, test_rmse, test_recall, test_map, high_test_rmse, high_test_recall, high_test_map),flush=True)
        ...
        if early_stop.early_stop:
            ...
            torch.save(net.state_dict(), f'logs/[{mode}] best_{data_type}_{epoch:05d}.pth')
            break
