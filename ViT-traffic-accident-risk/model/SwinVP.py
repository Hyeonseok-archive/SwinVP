...
from .SwinTransformer import StageModule
...
# SimVPv2 models #
##################
class GL3D(nn.Module):
    def __init__(self, c_in, c_out):
        super(GL3D, self).__init__()
        self.conv = nn.Conv3d(c_in, c_out, kernel_size=(1, 1, 1))
        self.sigconv = nn.Sequential(
            nn.Conv3d(c_in, c_out, kernel_size=(1, 1, 1)),
            nn.Sigmoid()
        )
        self.out = c_out
...
class SwinVP(nn.Module):
    ...
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 ...
                 spatio_kernel_dec=3, mode='swin', **kwargs):
        super(SwinVP, self).__init__()
        self.mode = mode
        T, C, H, W = in_shape  # T is pre_seq_length
        H, W = int(H / 2 ** (N_S / 2)), int(W / 2 ** (N_S / 2))  # downsample 1 / 2**(N_S/2)

        if mode == 'swin':
            self.header = GL3D(C, 1)

            self.enc = nn.Sequential(
                # StageModule(in_channels=1, hidden_dimension=hid_S, layers=4, downscaling_factor=1,
                #             num_heads=4, head_dim=hid_S, window_size=2),
                StageModule(in_channels=C, hidden_dimension=hid_S, layers=4, downscaling_factor=1,
                            num_heads=4, head_dim=hid_S, window_size=2),
                nn.BatchNorm2d(hid_S),
                nn.LeakyReLU(.2, inplace=True),

                StageModule(in_channels=hid_S, hidden_dimension=hid_S, layers=4, downscaling_factor=2,
                            num_heads=4, head_dim=hid_S, window_size=2),
                nn.BatchNorm2d(hid_S),
                nn.LeakyReLU(.2, inplace=True),

                # StageModule(in_channels=hid_S, hidden_dimension=hid_S, layers=4, downscaling_factor=1,
                #             num_heads=4, head_dim=hid_S, window_size=2),
                # nn.BatchNorm2d(hid_S),
                # nn.LeakyReLU(.2, inplace=True),

                StageModule(in_channels=hid_S, hidden_dimension=hid_S, layers=4, downscaling_factor=2,
                            num_heads=4, head_dim=hid_S, window_size=1),
                nn.BatchNorm2d(hid_S),
                nn.LeakyReLU(.2, inplace=True)
            )
        else:
